...
export interface GarantiaProps {
  ...
  icon?: LucideIcon;
  /** Sello/mockup real en vez del chip de ícono — cuando el usuario da su propia imagen. */
  sello?: string;
  id?: string;
}

export function Garantia({ nombre, condicionMarked, pisoLegal, icon: Icono = ShieldCheck, sello, id }: GarantiaProps) {
  warnCopy('Garantía → condición', condicionMarked, 30);
  const { contenedor, item } = useReveal();

  return (
    <SectionShell id={id} tinte="menta" compacta ariaLabel="Garantía">
      <motion.div
        variants={contenedor}
        initial="hidden"
        whileInView="visible"
        ...
        <motion.div variants={item}>
          ...
          <Hairline surface="surface" className="shadow-[var(--shadow-1)]">
            <div className="flex flex-col items-center gap-4 px-6 py-10 text-center">
              {sello ? (
                <img src={sello} alt={nombre} width={140} height={140} loading="lazy" className="h-32 w-32 object-contain" />
              ) : (
                <span
                  aria-hidden="true"
              ...
              <h2 className="text-balance text-[24px] font-bold leading-tight [font-family:var(--font-display)]">
                <Accent>{nombre}</Accent>
              </h2>
              <p className="max-w-[44ch] text-[16px] leading-[1.6] text-[var(--text-secondary)]">
