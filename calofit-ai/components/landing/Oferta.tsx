...
export interface OfertaProps {
  ...
  mensual: PlanOferta;
  /** Línea de urgencia bajo el CTA del plan anual (rojo, uso ÚNICO en toda la página). */
  ...
  return (
    <div>
      <p className="flex items-baseline gap-1">
        ...
        <span className="text-[16px] text-[var(--text-secondary)]">{plan.sufijo ?? '/mes'}</span>
      </p>
      {plan.descomposicionDia && (
        <p className="mt-1 text-[12px] text-[var(--text-secondary)]">{plan.descomposicionDia}</p>
      )}
    </div>
  );
}
      ...
      <motion.div variants={contenedor} initial="hidden" whileInView="visible" viewport={VIEWPORT_ONCE}>
        ...
        <div className="mx-auto mt-10 grid max-w-4xl grid-cols-1 items-start gap-6 md:grid-cols-2">
          ...
          <motion.div variants={item} className="relative md:-translate-y-2">
            {anual.badge && (
              <span className="absolute -top-[12px] left-1/2 z-10 -translate-x-1/2 rounded-full border border-[color-mix(in_oklab,var(--accent)_25%,transparent)] bg-[var(--accent)] px-3 py-1 text-[12px] font-bold uppercase tracking-[0.06em] text-[var(--bg)]">
                {anual.badge}
              </span>
            )}
            <Hairline emphasis surface="surface" className="shadow-[0_12px_36px_color-mix(in_oklab,var(--accent)_16%,transparent)]">
              <div className="rounded-[var(--radius-card)] bg-[color-mix(in_oklab,var(--accent)_5%,transparent)] p-6 md:p-7">
                <div className="flex items-center justify-between gap-3">
                  <h3 className="text-[16px] font-semibold text-[var(--text-primary)]">{anual.nombre}</h3>
                ...
                <div className="mt-6">
                  ...
                  {urgenciaLabel && (
                    <p className="mt-2 text-center text-[12px] font-bold uppercase tracking-[0.04em] text-[var(--urgencia)]">
                      {urgenciaLabel}
                    </p>
                  )}
                </div>
          ...
          >
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-[16px] font-semibold text-[var(--text-primary)]">{mensual.nombre}</h3>
              {trialDias !== undefined && <TrialBadge dias={trialDias} />}
            </div>
            <div className="mt-4">
              <Precio plan={mensual} />
