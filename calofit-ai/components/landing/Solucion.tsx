...
export interface SolucionProps {
  ...
  /** Antes/después opcional: split 2 columnas, el "después" con acento y glow. */
  antesDespues?: {
    labelAntes: string;
    /** 2-3 líneas — una por punto de dolor. */
    antes: [string, string, string];
    labelDespues: string;
    /** 2-3 líneas — una por punto de alivio. */
    despues: [string, string, string];
    /** Fotos reales opcionales — sin ellas, la card queda solo en texto (honesto). */
    imgAntes?: string;
    imgDespues?: string;
  };
  id?: string;
...
export function Solucion({
  ...
  return (
    <SectionShell id={id} tinte="menta" ariaLabel="Cómo funciona">
      <motion.div
        variants={contenedor}
        initial="hidden"
        whileInView="visible"
      ...
      >
        ...
        {antesDespues && (
          <motion.div variants={item} className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2">
            {/* ANTES: grises apagados + borde rojo semántico (bloqueo/obsoleto) */}
            <div className="overflow-hidden rounded-[var(--radius-card)] border-2 border-[var(--urgencia)] bg-[var(--surface-2)]">
              {antesDespues.imgAntes && (
                <img
                  src={antesDespues.imgAntes}
                  alt={antesDespues.labelAntes}
                  width={400}
                  height={300}
                  loading="lazy"
              ...
              <div className="p-5">
                <p className="text-[12px] font-semibold uppercase tracking-[0.08em] text-[var(--urgencia)]">
                  {antesDespues.labelAntes}
                </p>
                <ul className="mt-3 flex flex-col gap-2">
                  {antesDespues.antes.map((linea, i) => (
                    <li key={i} className="text-[16px] leading-snug text-[var(--text-secondary)]">
                      {linea}
                    </li>
                  ))}
                </ul>
            ...
            {/* DESPUÉS: fresco + borde verde con glow que respira (vivo, tecnológico) */}
            <div className="glow-despues overflow-hidden rounded-[var(--radius-card)] border-2 border-[var(--accent-2)] bg-[var(--surface)]">
              {antesDespues.imgDespues && (
                <img
                  src={antesDespues.imgDespues}
                  alt={antesDespues.labelDespues}
                  width={400}
                  height={300}
                  loading="lazy"
              ...
              <div className="p-5">
                ...
                <ul className="mt-3 flex flex-col gap-2">
                  {antesDespues.despues.map((linea, i) => (
                    <li key={i} className="text-[16px] font-medium leading-snug text-[var(--text-primary)]">
                      {linea}
                    </li>
                  ))}
                </ul>
